...
function imprimir() {
    ...
    document.getElementById( 'informesSur' )
...
function exportarCSV() {
    tipo = document.querySelector( 'input[name="radInforme"]:checked' )
        .value;
    var tipoInf = '';
    if ( tipo == 'cau' ) {
        tipoInf = 'Caudales';
...
//elimina los ajustes
function reset() {
    document.getElementById( 'espacioInforme' )
        .innerHTML = "";
}

//abre o cierra la cabeza de opciones para el informe
function opciones() {
    if ( document.getElementById( "informesNorte" )
        .style.height == '15%' ) {
        document.getElementById( "informesNorte" )
            .style.height = 0;
        document.getElementById( "btnMenuInformes" )
            .style.top = '6%';
...
function obtenerInforme() {
    ...
    var estaciones = $( '#opcionesEstacion' )
        .val();
    var nestaciones = [];
    for ( var est in estaciones ) {
        nestaciones[ estaciones[ est ] ] = document.getElementById( 'est' + estaciones[ est ] )
            .getAttribute( 'name' );
    }
<<<<<<< Updated upstream
    ...
    $( document )
        .ready( function () {
            $.ajax( {
                ...
                contentType: 'application/json;charset=utf-8',
                url: 'http://localhost/Aquando/public/A_Informes.php?opcion=' + opcion + '&fechaIni=' + fInicio + '&fechaFin=' + fFin,
                success: function ( informe ) {
                    reset();
                    var ahora = new Date();
                    var fechahora = "" + ahora.getDate() + "-" +
