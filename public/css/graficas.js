...
function limpiar() {
    ...
    tagsEstacion( document.getElementById( 'opciones' )
        .value );
    aplicarOpciones();
...
function metaDatosTag( id_tag, id_estacion ) {
    ...
    $.ajax( {
        type: 'GET',
        url: 'http://localhost/Aquando/public/A_Graficas.php?opcion=meta&tag=' + id_tag + '&estacion=' + id_estacion,
        success: function ( meta ) {
            ...
            $.ajax( {
                type: 'GET',
                url: 'http://localhost/Aquando/public/A_Graficas.php?estacion=' + id_estacion + '&tag=' + id_tag + '&opcion=render',
...
function aplicarOpciones() {
    var idEstacion = document.getElementById( "opciones" )
        .value;
    var idTag = document.getElementById( "opcionesTag" )
        .value;
    document.getElementById( 'compararSel' )
        .value = 'nada';
...
function tagsEstacion( id_estacion ) {
    ...
    $( document )
        .ready( function () {
            $.ajax( {
                type: 'GET',
                url: 'http://localhost/Aquando/public/A_Graficas.php?estacion=' + id_estacion + '&opcion=tags',
                success: function ( tags ) {
                    document.getElementById( "opcionesTag" )
                        .innerHTML = "";
                    document.getElementById( "compararSel" )
                        .innerHTML = "<option value='nada' selected>Nada</option>";
                    var e = 0;
                    sessionStorage.setItem( 'tagsAct', JSON.stringify( tags ) );
                    for ( var tag in tags ) {
                        if ( e == 0 ) {
                            document.getElementById( "opcionesTag" )
                                .innerHTML += "<option value=" + tags[ tag ][ 'id_tag' ] + " selected>" + tags[ tag ][ 'nombre_tag' ] + "</option>";
                        } else {
                            document.getElementById( "opcionesTag" )
                                .innerHTML += "<option value=" + tags[ tag ][ 'id_tag' ] + ">" + tags[ tag ][ 'nombre_tag' ] + "</option>";
                        }
                        document.getElementById( "compararSel" )
                            .innerHTML += "<option value=" + tags[ tag ][ 'id_tag' ] + ">" + tags[ tag ][ 'nombre_tag' ] + "</option>";
                        e++;
...
function renderGrafico( datosR ) {
    ...
    for ( var tindex in tagsAct[ 0 ] ) {
        if ( tagsAct[ 0 ][ tindex ][ 'id_tag' ] == document.getElementById( "opcionesTag" )
            .value || tagsAct[ 0 ][ tindex ][ 'id_tag' ] == document.getElementById( "compararSel" )
            .value ) {
    ...
    for ( var index in datosR ) {
        for ( var valor in datosR[ index ] ) {
            if ( valor != "fecha" && valor != "calidad" ) {
                valores.push( datosR[ index ][ valor ][ 'valor' ] );
            }
        }
    }
    console.log( valores );
...
function comparar() {
    ...
    if ( document.getElementById( "compararSel" )
        ...
        var idEstacionCom = document.getElementById( "opciones" )
            .value;
        var idTagCom = document.getElementById( "compararSel" )
            .value;
