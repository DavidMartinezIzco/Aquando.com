//limpia los filtros
function limpiar() {
    document.getElementById( "radioDesc" )
        .checked = 'checked';
    document.getElementById( "radioFecha" )
        .checked = 'checked';
...
function inicioFin() {
    ...
    Date.prototype.setearHasta = ( function () {
        var mesant = new Date( new Date()
            .getTime() - 30 * 24 * 60 * 60 * 1000 );
        return mesant.toJSON()
            .slice( 0, 10 );
    } );
    $( document )
        .ready( function () {
...
function opciones() {
    if ( document.getElementById( "zonaOpciones" )
        .style.height == '10%' ) {
        document.getElementById( "zonaOpciones" )
            .style.height = "0%";
        document.getElementById( "zonaAlarmas" )
            .style.maxHeight = '95%';
    } else {
        document.getElementById( "zonaOpciones" )
            .style.height = "10%";
    ...
    if ( id_estacion != 'all' ) {
        ...
        $( document )
            .ready( function () {
                $.ajax( {
                    type: 'GET',
                    url: 'http://localhost/Aquando/public/A_Alarmas.php?funcion=actualizar&nombre=' + nombre + '&pwd=' + pwd + '&emp=' + emp + '&sentido=' + sentido + '&orden=' + orden + '&fechaInicio=' + fechaInicio + '&fechaFin=' + fechaFin,
                    success: function ( alarmas ) {
                        document.getElementById( "tablaAlarmas" )
                            .innerHTML = alarmas;
                    },
                    error: function () {
                        console.log( "error" );
    ...
    if ( id_estacion == 'all' ) {
        ...
        $( document )
            .ready( function () {
                $.ajax( {
                    type: 'GET',
                    url: 'http://localhost/Aquando/public/A_Alarmas.php?funcion=estacion&sentido=' + sentido + '&orden=' + orden + '&estacion=' + id_estacion + '&fechaInicio=' + fechaInicio + '&fechaFin=' + fechaFin,
                    success: function ( alarmas ) {
                        document.getElementById( "tablaAlarmas" )
                            .innerHTML = alarmas;
                    },
...
function reconocer( id_alarma ) {
    ...
    $( document )
        .ready( function () {
            $.ajax( {
                type: 'GET',
                url: 'http://localhost/Aquando/public/A_Alarmas.php?funcion=reconocer&alarma=' + id_alarma + '&nombre=' + sessionStorage.getItem( 'nousu' ) + '&fecha_ack=',
                success: function () {
                    actualizar( null );
                },
                error: function () {
