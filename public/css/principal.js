...
function mapas() {
    ...
    for ( var index in estacionesUbis ) {
        var accion = 'http://localhost/Aquando/public/index.php/Inicio/estacion';
        ...
        if ( estacionesUbis[ index ][ 0 ][ 'foto' ] != null ) {
            ...
            estacion.bindPopup( imagen + msg + btn )
                .openPopup();
        } else {
            estacion.bindPopup( msg + btn )
                .openPopup();
        }
...
function actualizar() {
    ...
    $( document )
        .ready( function () {
            $.ajax( {
                type: 'GET',
                url: 'http://localhost/Aquando/public/A_Principal.php?opcion=refresh',
                data: {
                    arrdatos: arrdatos
                },
    ...
    if ( i > 0 ) {
        ...
        $( document )
            .ready( function () {
                $.ajax( {
                    type: 'GET',
                    url: 'http://localhost/Aquando/public/A_Principal.php?opcion=ajustes',
                    data: {
...
function confirmarAjustesWidget( wid ) {
    ...
    $( document )
        .ready( function () {
            $.ajax( {
                type: 'GET',
                url: 'http://localhost/Aquando/public/A_Principal.php?opcion=confirmar&wid=' + widget + '&tag=' + tag + '&usu=' + usu + '&pwd=' + pwd,
                success: function () {
...
function feedPrincipalCustom() {
    $( document )
        .ready( function () {
            $.ajax( {
                type: 'GET',
                url: 'http://localhost/Aquando/public/A_Principal.php?opcion=feed&usu=' + usu + '&pwd=' + pwd,
    ...
    for ( var wid in feed ) {
        ...
        if ( nombre_dato.includes( 'Nivel' ) || nombre_dato.includes( 'Acumulado' ) ) {
            ...
            if ( nombre_dato.includes( 'Acumulado' ) ) {
                ...
                widCon += "</tr></table>"
                document.getElementById( "gau" + feed[ wid ][ 'widget' ] )
                    .innerHTML = 'Valor de ' + nombre_dato + '<br> ' + widCon;
        ...
        if ( nombre_dato.includes( 'Acumulado' ) ) {
            ...
            for ( var index in agreg_semanal ) {
                max_agreg.push( parseFloat( agreg_semanal[ index ][ 'max' ] )
                    .toFixed( 2 ) );
                min_agreg.push( parseFloat( agreg_semanal[ index ][ 'min' ] )
                    .toFixed( 2 ) );
                avg_agreg.push( parseFloat( agreg_semanal[ index ][ 'avg' ] )
                    .toFixed( 2 ) );
                fechas_agreg.push( agreg_semanal[ index ][ 'fecha' ] );
            }
            optionSemanal = {
