...
function prepararTag( info, tag ) {
    ...
    for ( var meta in info[ 'meta' ] ) {
        ...
        if ( meta == 'min' ) {
            colorMeta = document.getElementById( "colorMin" )
                .value;
            marcaMeta[ 'name' ] = "minimo gen";
            marcaMeta[ 'lineStyle' ][ 'normal' ][ 'color' ] = colorMeta;
        }
        if ( meta == 'avg' ) {
            colorMeta = document.getElementById( "colorAvg" )
                .value;
    ...
    if ( document.getElementById( "checkMaxInt" )
        .checked ) {
        colorMeta = document.getElementById( "colorMaxInt" )
            .value;
        var marcaMeta = {};
        marcaMeta[ 'name' ] = 'max';
        marcaMeta[ 'type' ] = 'max';
    ...
    if ( document.getElementById( "checkMinInt" )
        .checked ) {
        colorMeta = document.getElementById( "colorMinInt" )
            .value;
        var marcaMeta = {};
        marcaMeta[ 'name' ] = 'min';
        marcaMeta[ 'type' ] = 'min';
    ...
    if ( document.getElementById( "checkAvgInt" )
        .checked ) {
        colorMeta = document.getElementById( "colorAvgInt" )
            .value;
        var marcaMeta = {};
...
function ajustesPresets( modo ) {
    ...
    if ( con.style.display == 'block' ) {
        ...
        if ( modo == 'cargar' ) {
            con.innerHTML = "";
            var pre = document.getElementById( 'selPresets' )
        ...
        $( document )
            .ready( function () {
                $.ajax( {
                    ...
                    url: '/Aquando.com/A_GraficasCustom.php?opcion=leerPresets&para=' + para,
                    data: {
                        arrdatos: arrdatos
                    },
                        ...
                        for ( var index in presets_config ) {
                            if ( presets_config[ index ][ 'configuracion' ].includes( n_preset ) ) {
                                ...
                                for ( var index in tagsycolores ) {
                                    ...
                                    document.getElementById( info[ 0 ] )
                                        .checked = 'true';
                                    document.getElementById( 'color' + info[ 0 ] )
                                        .value = info[ 1 ];
                                    if ( document.getElementById( info[ 0 ] )
